...
| [`$se:study`](../plugins/se/skills/study/SKILL.md), [`$se:spec`](../plugins/se/skills/spec/SKILL.md) | `gpt-5.6-luna` | `max` | Shared [`evidence-researcher`](../plugins/se/references/subagents.md#evidence-researcher) role. Calling skills own selection, concurrency, lifecycle, and fallback. |
...
| [`$se:deliver-features`](../plugins/se/skills/deliver-features/SKILL.md) | `configured/default` | `configured/default` | One visible graph orchestrator in the single involved project or the caller-selected coordination project; the skill intentionally inherits the configured profile unless the caller explicitly overrides it. |
| [`$se:deliver-features`](../plugins/se/skills/deliver-features/SKILL.md) | `configured/default` | `configured/default` | Reusable repository-bound worker lanes in isolated worktrees; the orchestrator chooses serial reuse or concurrent lanes without selecting a fixed profile. |
| [`$se:deliver-features`](../plugins/se/skills/deliver-features/SKILL.md) | `gpt-5.6-sol` | `xhigh` | Fresh independent read-only local reviewer execution for each stable committed delivery-unit candidate; the review covers the complete unit delta and spec/task contribution and must be clean for the exact base and full HEAD before publication. |
