...
- Treat plain-language "upgrade this skill" requests as targeted-skill maintenance first; only expand to repo-wide refresh flows when the user asks for them explicitly.
- Treat generic `$skills-maintainer` imperatives like "run" or "run your tasks" as a proactive maintenance pass across existing skills: inspect the repo, choose skills with clear actionable drift, upgrade them, then run sync, audit, and `release-checklist.md`; do not infer new-skill creation or `refresh`. (Codex learning)
