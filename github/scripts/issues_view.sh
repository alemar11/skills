...
usage() {
  cat <<'EOF'
Usage: issues_view.sh --issue <number> [--repo <owner/repo>] [--allow-non-project]
EOF
}
...
gh issue view "$ISSUE" --repo "$TARGET_REPO" --json number,title,state,labels,assignees,body,createdAt,updatedAt,url,author,comments --jq '.'
