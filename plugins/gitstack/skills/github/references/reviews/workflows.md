...
- Run `ghops --json doctor` when repo context is uncertain before
  write operations.

### Operator policy

- Prefer `ghops reviews address` over manual `gh api graphql`
  rollups.
...
```bash
ghops --json reviews address --pr <number> [--repo <owner/repo>]
```
...
- Prefer `ghops reviews address --reply-body ... --dry-run` before a
  real write.
- Use either `--selection` or `--comment-ids` with `--reply-body`.
- Use `ghops reviews comment` for top-level PR comments when a direct
...
```bash
ghops reviews address --pr <number> --selection <rows> --reply-body <text> --dry-run [--repo <owner/repo>]
ghops reviews comment --pr <number> --body <text> [--repo <owner/repo>]
```
...
- Use `ghops reviews review` instead of manual `gh pr review` when you
...
```bash
ghops reviews review --pr <number> [--approve|--request-changes|--comment] [--body <text>] [--repo <owner/repo>]
```
...
- Reply-target ambiguity: rerun `ghops --json reviews address --pr <number>`
