...
```bash
ghops --json stars list [--by-list <slug-or-name>|--list-id <id>] [--limit N|--all]
ghops --json stars add --repo <owner/repo> [--repo <owner/repo>]... [--repos-file <path>] [--dry-run]
ghops --json stars remove --repo <owner/repo> [--repo <owner/repo>]... [--repos-file <path>] [--dry-run]
```
...
```bash
ghops --json lists list [--limit N|--all]
ghops --json lists items --list <slug-or-name>|--list-id <id> [--limit N|--all]
ghops --json lists create --name <text> [--description <text>] [--visibility private|public] [--dry-run]
ghops --json lists delete --list <slug-or-name>|--list-id <id> [--dry-run]
ghops --json lists assign --list <slug-or-name>|--list-id <id> --repo <owner/repo> [--repo <owner/repo>]... [--repos-file <path>] [--dry-run]
ghops --json lists unassign --list <slug-or-name>|--list-id <id> --repo <owner/repo> [--repo <owner/repo>]... [--repos-file <path>] [--dry-run]
```
...
- Prefer `ghops prs update` over ad-hoc `gh pr edit`.
...
- Use `ghops --json doctor` first when repo context is uncertain.

### Preferred command

```bash
ghops prs update --pr <number> [--title <text>] [--body <text>] [--base <branch>] [--repo <owner/repo>]
```
...
```bash
ghops issues copy --issue <number> --source-repo <owner/repo> --target-repo <owner/repo> [--dry-run]
ghops issues move --issue <number> --source-repo <owner/repo> --target-repo <owner/repo> [--dry-run]
```
...
```bash
ghops issues close-with-evidence --issue <number> --commit-sha <sha> [--commit-url <url>] [--pr-url <url>] [--repo <owner/repo>] [--dry-run]
```
...
- Prefer `ghops prs patch` over ad-hoc pull-request file API calls.
- Use `--path` when the user only cares about one file.

### Preferred command

```bash
ghops --json prs patch --pr <number> [--repo <owner/repo>] [--path <file>] [--include-patch]
```
...
```bash
ghops --json reactions list --resource pr|issue|issue-comment|pr-review-comment --repo <owner/repo> [--number <n>|--comment-id <id>]
ghops --json reactions add <reaction> --resource pr|issue|issue-comment|pr-review-comment --repo <owner/repo> [--number <n>|--comment-id <id>] [--dry-run]
ghops --json reactions remove <reaction-id> --resource pr|issue|issue-comment|pr-review-comment --repo <owner/repo> [--number <n>|--comment-id <id>] [--dry-run]
```
...
```bash
ghops --json issues suggest-labels --repo <owner/repo> --title <text> [--body <text>] [--max-suggestions N] [--min-score <float>] [--allow-new-label] [--new-label-color <rrggbb>] [--new-label-description <text>]
```
