...
```bash
ghops issues close-with-evidence --issue <number> --commit-sha <sha> [--commit-url <url>] [--pr-url <url>] [--repo <owner/repo>] [--allow-non-project] [--dry-run]
```
...
- Use `ghops issues comment --issue <number> --repo <owner/repo> --body <text>`
...
- Prefer `ghops issues copy` when the source issue should stay open and
  work should continue in both places.
- Prefer `ghops issues move` when work should continue only in the
  target repository.
